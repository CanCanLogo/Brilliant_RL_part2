...
class MDP:
        ...
        while iterId < nIterations:
            ...
            for state in range(self.nStates):
                ...
                for action in range(self.nActions):
                    # value_temp = np.dot(self.T[action][state], self.R[action] + self.discount * V)
                    # max_value = max(max_value, value_temp)
            ...
            # epsilon = np.sqrt(np.sum(np.power(V - V_new, 2)))
    ...
    def extractPolicy(self,V):
        ...
        for state in range(self.nStates):
            value_a_iter = []
            for action in range(self.nActions):
                # value_temp = np.dot(self.T[action][state], self.R[action] + np.multiply(self.discount, V))
                # print(self.R[action].shape)
                # print(self.T[action][state].shape)
                # print(V.shape)
